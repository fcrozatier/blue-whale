...
import { type Lexer, compile, patternToString } from "../blue-whale";

function lexAll(lexer: Lexer) {
	return Array.from(lexer);
}

describe("regex building", () => {
	test("builds", () => {
		const pattern = [/\d/, /\d\.\d/];
		expect(patternToString(pattern)).toBe("\\d|\\d\\.\\d");
	});
});
...
// 		const lexer = compile({
// 			apples: "a",
// 			name: { match: /[a-z]/, type: keywords({ kw: ["m"] }) },
// 		}).reset("azm");
